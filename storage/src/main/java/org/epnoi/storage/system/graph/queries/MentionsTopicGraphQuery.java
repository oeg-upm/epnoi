...
public class MentionsTopicGraphQuery implements GraphQuery<MentionsFromTopic> {
    ...
    @Override
    public List<MentionsFromTopic> execute(String startUri, String endUri) {
        Map<String,String> params = new HashMap<>();
        params.put("0",startUri);
        params.put("1",endUri);
        Result result = executor.execute("match (node1:Topic{uri:{0}})-[r:MENTIONS]->(node2:Word{uri:{1}}) return r", params);
