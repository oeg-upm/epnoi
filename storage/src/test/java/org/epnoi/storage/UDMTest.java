...
import org.epnoi.storage.documents.Reference;
...
public class UDMTest {
    ...
    public void listOfDocuments() throws IOException {
        ...
        List<Item> items = udm.find(Resource.Type.ITEM).all().stream().map(uri -> udm.read(Resource.Type.ITEM).byUri(uri).get().asItem()).collect(Collectors.toList());

//		items.forEach(item-> LOG.info("Item: " + item.getUri() + " -> " + item.getUrl()));

		LOG.info("Items: " + items);

        java.io.File baseFolder = new File("/Users/cbadenes/Documents/OEG/Projects/DrInventor/datasets/2nd-review");
        List<String> files = FolderUtils.listFiles(baseFolder);

        HashMap<String,String> fileTable = new HashMap<>();

        files.stream().map(path -> new java.io.File(path)).filter(file -> refFiles.contains(file.getName())).forEach(file->fileTable.put(file.getName(), StringUtils.substringAfter(file.getAbsolutePath(),baseFolder.getAbsolutePath())));
