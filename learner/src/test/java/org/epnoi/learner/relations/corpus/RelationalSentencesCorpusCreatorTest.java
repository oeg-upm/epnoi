...
public class RelationalSentencesCorpusCreatorTest {

    private static final Logger logger = LoggerFactory.getLogger(RelationalSentencesCorpusCreatorTest.class);
...
//            }
    ...
    public boolean _isValid(gate.Annotation sentenceAnnotation) {
    ...
    public void _searchDocument(Document document) {
        ...
        gate.AnnotationSet sentenceAnnotations = document.getAnnotations().get(NLPAnnotationsConstants.SENTENCE);
        DocumentContent sentenceContent = null;
        gate.AnnotationSet sentencesAnnotations = document.getAnnotations();
        Iterator<gate.Annotation> sentencesIt = sentenceAnnotations.iterator();
        while (sentencesIt.hasNext()) {
            gate.Annotation sentenceAnnotation = sentencesIt.next();
    ...
    private void _searchSentence(Document document, gate.AnnotationSet sentencesAnnotations, gate.Annotation sentenceAnnotation) {
    ...
    private void _testSentence(gate.Annotation sentence, DocumentContent sentenceContent,
                               gate.AnnotationSet sentenceAnnotationsSet) {
        ...
        Map<String, gate.Annotation> termsAnnotationsTable = _createTermsAnnotationsTable(sentenceContent,
    ...
    private void _createRelationalSentence(DocumentContent sentenceContent, Long sentenceStartOffset,
                                           Map<String, gate.Annotation> termsAnnotationsTable, String term, Set<String> termHypernyms) {
        gate.Annotation sourceTermAnnotation = termsAnnotationsTable.get(term);
        ...
        for (String destinationTerm : termHypernyms) {
            ...
            gate.Annotation destinationTermAnnotation = termsAnnotationsTable.get(destinationTerm);
            ...
            try {
                annotatedContent = nlpHandler.process(sentenceContent.toString());
            } catch (EpnoiResourceAccessException e) {
    ...
    private Map<String, gate.Annotation> _createTermsAnnotationsTable(DocumentContent sentenceContent,
                                                                      gate.AnnotationSet sentenceAnnotationsSet, Long sentenceStartOffset, Set<String> sentenceTerms) {
        HashMap<String, gate.Annotation> termsAnnotationsTable = new HashMap<String, gate.Annotation>();
        for (gate.Annotation termAnnotation : sentenceAnnotationsSet.get(NLPAnnotationsConstants.TERM_CANDIDATE)) {
    ...
    private void _addTermToTermsTable(Set<String> sentenceTerms, HashMap<String, gate.Annotation> termsAnnotationsTable,
                                      gate.Annotation termAnnotation, String term) {
        if (term.length() > 2) {
    ...
    public void simpleTest() {
        ...
        //TODO
        logger.error("pending to implement by using UDM");
//        RelationalSentencesCorpus relationalSentenceCorpus = (RelationalSentencesCorpus) core.getInformationHandler().get(relationalCorpusURI, RDFHelper.RELATIONAL_SENTECES_CORPUS_CLASS);
        RelationalSentencesCorpus relationalSentenceCorpus = null;
    ...
    public RelationalSentencesCorpus createTestCorpus() {
        ...
        try {
            annotatedContentA = nlpHandler.process("A dog is a canine");
        ...
        try {
            annotatedContentB = nlpHandler.process("A dog, is a canine (and other things!)");
        } catch (EpnoiResourceAccessException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
